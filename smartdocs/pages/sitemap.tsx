...
export default function SitemapPage({ components }: SitemapProps) {
  const [expandedNodes, setExpandedNodes] = useState<Set<string>>(new Set(['src/', '/src/pages/', '/src/components/', '/src/hooks/', '/src/api/']))
  ...
  const renderProjectNode = (node: ProjectNode, depth = 0, isLast = false, parentPath = '') => {
    ...
    return (
      <div key={nodeId} className="relative">
        ...
        <div 
          className="flex items-center group hover:bg-slate-50 dark:hover:bg-slate-900/50 rounded-lg p-2 transition-colors relative"
        ...
        >
          ...
          {hasChildren && (
            <button
              ...
              className="mr-2 p-0.5 hover:bg-slate-200 dark:hover:bg-slate-700 rounded transition-colors"
            >
              <ChevronRight 
          ...
          <div className="flex-1">
            {node.path ? (
              ...
              >
                ...
                <ExternalLink className="h-3 w-3 text-slate-400 opacity-0 group-hover:opacity-100 transition-opacity ml-2" />
              </Link>
            ) : (
              <div>
                <span className={`font-medium ${
          ...
          {/* File path indicator */}
          {node.filePath && (
            <div className="ml-2 px-2 py-1 bg-slate-100 dark:bg-slate-800 rounded text-xs text-slate-500 dark:text-slate-400 font-mono opacity-0 group-hover:opacity-100 transition-opacity">
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="relative">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              <div className="p-3 rounded-xl bg-gradient-to-r from-slate-500 to-slate-600 shadow-lg">
                <GitBranch className="h-6 w-6 text-white" />
              </div>
              <div>
                <h1 className="text-4xl font-bold bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 dark:from-slate-100 dark:via-slate-300 dark:to-slate-100 bg-clip-text text-transparent">
                  🗂️ Project Structure
                </h1>
          ...
          <div className="flex flex-wrap items-center gap-3 mt-6">
            {[
              ...
              <div key={item.type} className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium ${item.color} border border-current/20`}>
                <item.icon className="h-4 w-4" />
                <span>{item.count} {item.type}{item.count !== 1 ? 's' : ''}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Legend */}
      <div className="bg-gradient-to-r from-slate-50 to-blue-50 dark:from-slate-900 dark:to-blue-900/20 rounded-2xl p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="flex items-center gap-3 p-3 bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700">
            ...
            <div>
              <div className="font-medium text-slate-900 dark:text-slate-100">Components</div>
          ...
          <div className="flex items-center gap-3 p-3 bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700">
            ...
            <div>
              <div className="font-medium text-slate-900 dark:text-slate-100">Hooks</div>
          ...
          <div className="flex items-center gap-3 p-3 bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700">
            ...
            <div>
              <div className="font-medium text-slate-900 dark:text-slate-100">Pages</div>
          ...
          <div className="flex items-center gap-3 p-3 bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700">
            ...
            <div>
              <div className="font-medium text-slate-900 dark:text-slate-100">APIs</div>
